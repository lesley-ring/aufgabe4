...
public class MessageListenerThread extends Thread {
				...
				case NOT_LOGGED_IN:
					...
					case LOGIN_REQUEST:
						if (server.checkLogin(message.getParamA(),
							...
							sendMessageChecked(new M2ProtocolMessage(
				...
				case LOGGED_IN:
					...
					case MESSAGE_REQUEST:
						if(message.getParamB().isEmpty())
							server.broadcastMessage(new M2ProtocolMessage(MessageType.MESSAGE_BROADCAST, name, message.getParamA()));
						else
	...
	public void sendMessage(M2ProtocolMessage message) {
		if (status == CommStatus.LOGGED_IN) {
			sendMessageChecked(message);
		}
	}

	private void sendMessageChecked(M2ProtocolMessage message) {
		if (output != null) {
			try {
				synchronized (output) {
