...
public class Server implements ICommWindowCallback {
	...
	public void broadcastMessage(M2ProtocolMessage message) {
	...
	public void relayMessage(M2ProtocolMessage message, MessageListenerThread source) {
		synchronized (clients) {
			for (MessageListenerThread client : clients) {
				if(client.getClientName().equals(message.getParamB()) || client.getClientName().equals(source.getClientName()))
					client.sendMessage(new M2ProtocolMessage(MessageType.MESSAGE_BROADCAST, source.getClientName(), message.getParamA()));
			}
		}
	}
